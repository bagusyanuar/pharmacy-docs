...
Berikutnya kita akan menyusun spesifikasi **Fase 1: Master Data, Multi-Branch & Autentikasi**:
...
2. ⏳ **`features/master-data/02-prd-obat-dan-satuan-bertingkat.md`**
   - *Cakupan Bisnis:* Katalog obat paten & generik, golongan obat (Bebas/Terbatas/Keras/Narkotika), hierarki multi-satuan bertingkat (Box $\rightarrow$ Strip $\rightarrow$ Tab), dan penentuan harga jual eceran.
3. ⏳ **`features/master-data/03-prd-pbf-supplier-dan-dokter.md`**
   - *Cakupan Bisnis:* Master distributor PBF, kontak, izin PBF, default TOP kredit, dan master dokter perujuk (No SIP).
4. ⏳ **`technical/01-dra-database-erd-master-data.md`**
   - *Cakupan Teknis:* Skema DDL PostgreSQL 15+ (tabel cabang, user, obat, multi-satuan, PBF), Foreign Key `ON DELETE RESTRICT`, invarian `branch_id`, dan diagram Mermaid ERD.
5. ⏳ **`technical/02-trd-master-data-api.md`**
...
> *"Halo Tika, tolong baca `PROGRESS.md` dan kita mulai eksekusi Fase 1: `features/master-data/01-prd-cabang-dan-gudang.md`."*
