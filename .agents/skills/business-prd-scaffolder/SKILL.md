...
* ✅ **User Journey & Experience:** Step-by-step narrative and visual Mermaid user journeys from the perspective of what pharmacy staff observe and execute on the screen. Any Mermaid diagram MUST follow the GitHub rendering standards (no parentheses `()`, slashes `/`, comparison operators `<>` or quotes inside edge labels `|...|`, and safe double-quoting of node labels).
